/* ============================================================
   ANIMALT — color space utilities
   PALETALT-K の color.js をそのまま流用（量子化の意味を変えないため）。

   量子化はすべて「ワークスペース座標」上で行う。
   OKLAB / RGB のどちらを選んでも、そこでのユークリッド距離が
   同一スケール（黒→白でおよそ 100）になるよう正規化してある。
...
const Color = (() => {
  ...
  function lightness(r, g, b) { return rgbToOklab(r, g, b)[0]; }
  function hue(r, g, b) {
    const [, A, B] = rgbToOklab(r, g, b);
...
if (typeof self !== 'undefined' && typeof window === 'undefined') self.Color = Color;
